<!-- Header -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=220&section=header&text=cocodding0723&fontSize=65&fontColor=fff&fontAlignY=38&desc=Unity+%7C+Flutter+Developer&descAlignY=58&descSize=22"/>
</div>

<!-- Typing animation -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=FF0080&center=true&vCenter=true&width=500&lines=Unity+Game+Developer;Flutter+App+Developer;Unreal+Engine+%7C+C%23+%7C+Dart;Backend+%26+DevOps+Engineer;Always+Building+Something" alt="Typing SVG" />
</div>

<br/>

<div align="center">
  <a href="https://github.com/cocodding0723">
    <img src="https://img.shields.io/github/followers/cocodding0723?label=Followers&style=for-the-badge&logo=github&color=7928CA&labelColor=1a1a2e"/>
  </a>
  <img src="https://komarev.com/ghpvc/?username=cocodding0723&style=for-the-badge&color=FF0080&label=PROFILE+VIEWS"/>
  <img src="https://img.shields.io/badge/Focus-Unity%20%26%20Flutter-blue?style=for-the-badge&labelColor=1a1a2e"/>
</div>

<br/>

---

## 🙋 About Me

```dart
class Developer {
  final String name    = "cocodding0723";
  final String role    = "Unity & Flutter Developer";
  final List<String> skills = [
    "Unity / Unreal Engine",  "Flutter / Dart",
    "React",                  "Java / Kotlin / C# / C++",
    "AWS / Docker / Jenkins", "PostgreSQL / Redis / Firebase",
  ];
  final String motto = "Code every day, grow every day 🚀";
}
```

<br/>

---

## 🛠 Tech Stack

<div align="center">
<table>
<tr><td align="center"><b>🎮 Game Dev</b></td><td>
<img src="https://skillicons.dev/icons?i=unity,unrealengine&theme=dark&perline=6"/>
</td></tr>
<tr><td align="center"><b>📱 Mobile & App</b></td><td>
<img src="https://skillicons.dev/icons?i=flutter,dart,androidstudio,react&theme=dark&perline=6"/>
</td></tr>
<tr><td align="center"><b>☁️ Backend</b></td><td>
<img src="https://skillicons.dev/icons?i=firebase,supabase,aws,docker,jenkins,postgres,redis&theme=dark&perline=7"/>
</td></tr>
<tr><td align="center"><b>🛠 Tools</b></td><td>
<img src="https://skillicons.dev/icons?i=git,github,vscode,rider&theme=dark&perline=6"/>
</td></tr>
<tr><td align="center"><b>💬 Languages</b></td><td>
<img src="https://skillicons.dev/icons?i=java,cs,cpp,python,kotlin,ts&theme=dark&perline=6"/>
</td></tr>
</table>
</div>

<br/>

---

## 📊 GitHub Stats

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-streak-stats-eight.vercel.app?user=cocodding0723&theme=radical&v=2"/>
    <source media="(prefers-color-scheme: light)" srcset="https://streak-stats.demolab.com?user=cocodding0723&theme=github&hide_border=true&stroke=7928CA&ring=7928CA&fire=FF0080"/>
    <img src="https://streak-stats.demolab.com?user=cocodding0723&theme=radical&hide_border=true&background=0D1117&stroke=FF0080&ring=7928CA&fire=FF0080&currStreakNum=ffffff&sideNums=ffffff&currStreakLabel=FF0080&sideLabels=7928CA&dates=ffffff" alt="GitHub Streak"/>
  </picture>
</div>

<br/>

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-activity-graph.vercel.app/graph?username=cocodding0723&theme=react-dark&bg_color=0D1117&color=FF0080&line=7928CA&point=FFD700&area=true&hide_border=true"/>
    <source media="(prefers-color-scheme: light)" srcset="https://github-readme-activity-graph.vercel.app/graph?username=cocodding0723&bg_color=ffffff&color=555555&line=7928CA&point=FF0080&area=true&hide_border=true"/>
    <img src="https://github-readme-activity-graph.vercel.app/graph?username=cocodding0723&theme=react-dark&bg_color=0D1117&color=FF0080&line=7928CA&point=FFD700&area=true&hide_border=true" alt="Activity Graph"/>
  </picture>
</div>

<br/>

---

## 🐍 Snake Game

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/cocodding0723/cocodding0723/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/cocodding0723/cocodding0723/output/github-contribution-grid-snake.svg">
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/cocodding0723/cocodding0723/output/github-contribution-grid-snake.svg">
  </picture>
</div>

<br/>

---

## 🎯 What I Build

<div align="center">

| 🎮 Game | 📱 Mobile | ☁️ Backend |
|:---:|:---:|:---:|
| Unity 3D/2D | Flutter (iOS/Android) | REST API |
| Unreal Engine | React Web/App | AWS · Docker |
| C# Game Systems | Clean Arch (MVVM) | PostgreSQL · Redis |
| Physics & Animation | Firebase / Supabase | CI/CD with Jenkins |

</div>

<br/>

---

<!-- Footer -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=20,11,6&height=120&section=footer"/>
</div>

<div align="center">
  <i>⭐ If you find my work interesting, feel free to star my repositories! ⭐</i>
</div>
